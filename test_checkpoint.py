    ...
    def run_one(result_dir, iter, idx, command):
        ...
        if RECORD_VIDEO:
            RECORD_DIR = os.path.join(
                current_dir, f"{RECORD_BASE_DIR}/{result_dir}/iter{iter}/idx{idx}"
            )
            command += f" enable_recording=True recording_dir={RECORD_DIR}"
        if not os.path.exists(f"{result_dir}/iter{iter}/{idx}/nn") or not os.listdir(
            f"{result_dir}/iter{iter}/{idx}/nn"
        ):
            raise FileNotFoundError(f"{result_dir}/iter{iter}/{idx}/nn")
        CHECKPOINT_NAME = sorted(
            [
                x
                for x in os.listdir(f"{result_dir}/iter{iter}/{idx}/nn")
                if os.path.isfile(os.path.join(f"{result_dir}/iter{iter}/{idx}/nn", x))
            ],
        ...
        checkpoint_path = (
            f"{current_dir}/{result_dir}/iter{iter}/{idx}/nn/{CHECKPOINT_NAME}"
        )
