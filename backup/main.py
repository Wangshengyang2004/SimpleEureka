...
from utils.simple_eureka import clean_response, remove_old_functions, final_cleaner, add_imports
...
def request_and_clean(cfg: DictConfig, full_prompt: str, original_crazyflie_code: str, task_id: int):
    """
    For one completion
    Request the response from the model and clean the response
    """
    client = OpenAI(
        api_key=cfg.api.key,
...
def main(cfg: DictConfig) -> None:
    ...
    if response := completion.choices[0].message.content:
        ...
        # Save the response to a file under {EUREKA_ROOT_DIR}/results
        with open(f"{EUREKA_ROOT_DIR}/results/{now}/response.txt", "w") as f:
            f.write(response)
        if re.search(r"#END", response):
            logger.success("Sucessfully parse response with #END")
    else:
