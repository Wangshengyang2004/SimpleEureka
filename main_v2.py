...
def main(cfg: DictConfig) -> None:
    ...
    for iter in range(cfg.generation.epochs):
        ...
        if cfg.generation.sample == 1:
            logger.info(f"Iteration {iter}: GPT Output:\n " + responses[0]["message"]["content"] + "\n")

        # logger Token Information
        logger.info(f"Iteration {iter}: Prompt Tokens: {prompt_tokens}, Completion Tokens: {total_completion_token}, Total Tokens: {total_token}")
        ...
        for response_id in range(cfg.generation.sample):
            ...
            with open(std_path, 'w') as f:
                ...
                process = subprocess.Popen(command, shell=True, stdout=f, stderr=f, encoding=encoding)
            block_until_training(std_path, success_keyword=cfg.env.success_keyword, failure_keyword=cfg.env.failure_keyword,
        ...
        if not exec_success and cfg.generation.sample != 1:
            ...
            logger.info("All code generation failed! Repeat this iteration from the current message checkpoint!")
            continue
        ...
        execute_rate = np.sum(np.array(successes) >= 0.) / cfg.generation.sample
        ...
        logger.info(f"Iteration {iter}: Max Success: {max_success}, Execute Rate: {execute_rate}, Max Success Reward Correlation: {max_success_reward_correlation}")
        logger.info(f"Iteration {iter}: Best Generation ID: {best_sample_idx}")
        logger.info(f"Iteration {iter}: GPT Output Content:\n" +  responses[best_sample_idx]["message"]["content"] + "\n")
        logger.info(f"Iteration {iter}: User Content:\n" + best_content + "\n")
            
        # Plot the success rate
        fig, axs = plt.subplots(2, figsize=(6, 6))
        fig.suptitle(f'{cfg.gym.task}')
    ...
    if max_reward_code_path is None: 
        ...
        logger.info("Please double check the output env_iter*_response*.txt files for repeating errors!")
        exit()
    logger.info(f"Task: {cfg.gym.task}, Max Training Success {max_success_overall}, Correlation {max_success_reward_correlation_overall}, Best Reward Code Path: {max_reward_code_path}")
